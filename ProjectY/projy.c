...
void *agentOne(){
  for (int i = 0; i < 6; i++) {
    sleep((rand()%200)/1000); //Sleep thread for up to 200 ms to mix things up
    sem_wait(&agentSem); //Wait until previous agent deal is finished
    printf("      Tobacco and paper generated by agent.\n");
...
void *agentTwo(){
  for (int i = 0; i < 6; i++) {
    sleep((rand()%200)/1000); //Sleep thread for up to 200 ms to mix things up
    sem_wait(&agentSem); //Wait until previous agent deal is finished
    printf("      Paper and match generated by agent.\n");
    sem_post(&paper); //wake up a paper pusher
...
void *agentThree(){
  for (int i = 0; i < 6; i++) {
    sleep((rand()%200)/1000); //Sleep thread for up to 200 ms to mix things up
    sem_wait(&agentSem); //Wait until previous agent deal is finished
    printf("      Match and tobacco generated by agent.\n");
    sem_post(&match); //wake up a match pusher
    sem_post(&tobacco); //wake up a tobacco pusher
  }
}
...
void *tobaccoPusher(){
  for (int i = 0; i < 12; i++) {
    ...
    if (isPaper) {
        ...
        isPaper = 0;
        printf("   Tobacco pusher sees paper, wakes match-holder\n");
        sem_post(&matchSem);
    }
    else if (isMatch) {
        // Matches and Tobacco are on the table - wake up paper
        isMatch = 0;
        printf("   Tobacco pusher sees match, wakes paper-holder\n");
        sem_post(&paperSem);
    }
    else { //Nothing else on the table - put down tobacco
        isTobacco = 1;
        printf("   Tobacco pusher sees no other supplies, puts tobacco on table.\n");
    }
    // give back mutex
    sem_post(&mutex);
  }
...
void *matchPusher(){
  for (int i = 0; i < 12; i++) {
    sem_wait(&match);  // Wakes up when an agent provides matches
    sem_wait(&mutex); // Must have exclusive access to table from other pushers
    if (isPaper) {
        // Paper and matches are on the table - wake up tobacco
        isPaper = 0;
        printf("   Match pusher sees paper, wakes tobacco-holder\n");
        sem_post(&tobaccoSem);
    }
    else if (isTobacco) {
        // Matches and Tobacco are on the table - wake up paper
        isTobacco = 0;
        printf("   Match pusher sees tobacco, wakes paper-holder\n");
        sem_post(&paperSem);
    }
    else { //Nothing else on the table - put down matches
        isMatch = 1;
        printf("   Match pusher sees no other supplies, puts matches on table.\n");
    }
    // give back mutex
    sem_post(&mutex);
  }
...
void *paperPusher(){
  for (int i = 0; i < 12; i++) {
    ...
    if (isTobacco) {
        // Paper and Tobacco are on the table - wake up matches
        isTobacco = 0;
        printf("   Paper pusher sees tobacco, wakes match-holder\n");
        sem_post(&matchSem);
    }
    else if (isMatch) {
        // Matches and paper are on the table - wake up tobacco
        isMatch = 0;
        printf("   Paper pusher sees match, wakes tobacco-holder\n");
        sem_post(&tobaccoSem);
    }
    else { //Nothing else on the table - put down paper
      isPaper = 1;
      printf("   Paper pusher sees no other supplies, puts paper on table.\n");
    }
    // give back mutex
    sem_post(&mutex);
  }
...
void *tobaccoSmoker(void* arg){
  ...
  for (int i = 0; i < 3; i++) {
    sem_wait(&tobaccoSem);
    sleep((rand()%50)/1000); //Make cigarette - Sleep thread for up to 50
    sem_post(&agentSem); //need new agent to provide more ingredients
    printf("Smoker %d (tobacco-holding) smokes cigarette %d\n", num, i);
    sleep((rand()%50)/1000);  //Smoke cigarette - Sleep thread for up to 50
  }
  printf("Smoker %d (tobacco-holding) is hungry, exits\n", num);
}
...
void *matchSmoker(void* arg){
  ...
  for (int i = 0; i < 3; i++) {
    ...
    sleep((rand()%50)/1000); //Make cigarette - Sleep thread for up to 50
    sem_post(&agentSem); //need new agent to provide more ingredients
    printf("Smoker %d (match-holding) smokes cigarette %d\n", num, i);
    sleep((rand()%50)/1000);  //Smoke cigarette - Sleep thread for up to 50
  }
  printf("Smoker %d (match-holding) is hungry, exits\n", num);
}
...
void *paperSmoker(void* arg){
  ...
  for (int i = 0; i < 3; i++) {
   ...
   sleep((rand()%50)/1000); //Make cigarette - Sleep thread for up to 50
   sem_post(&agentSem); //need new agent to provide more ingredients
   printf("Smoker %d (paper-holding) smokes cigarette %d\n", num, i);
   sleep((rand()%50)/1000); //Smoke cigarette - Sleep thread for up to 50
  }
  printf("Smoker %d (paper-holding) is hungry, exits\n", num);
}
